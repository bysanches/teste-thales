...
const Calculadora /* : FunctionComponent<Props> */ = ({
  resultadoAlterado,
}) => {
  ...
  const calcular = (evento) => {
    ...
    const { resultado, tipoDeOperacao } = calculoGeral(
      selectedSimbolo,
      simbolos
    );
    ...
    if (resultado !== undefined) {
      ...
      resultadoAlterado(resultadoMedia, resultado, {
        resultado,
      });
      ...
      console.error(
        "Erro no cálculo. Certifique-se de que todos os campos estão preenchidos corretamente."
      );
    }
  };
